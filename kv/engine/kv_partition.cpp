...
bool KVPartition::ScanLocalPartition(
  ...
  // §3.9.1: yield the private tree to any in-flight range migrate on this
  // partition (ScanLocal contends on the same private leaves as move_in).
  if (ScanRangeMigrateInFlight()) return false;
  auto *table = private_table_.get();
  if (table == nullptr)
