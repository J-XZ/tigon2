...
static int ivpci_request_msix_vectors(struct ivpci_private *ivpci_dev, int n)
{
	...
	ivpci_dev->msix_entries = kmalloc(n * sizeof(struct msix_entry), GFP_KERNEL);
	if (ivpci_dev->msix_entries == NULL) {
		ret = -ENOMEM;
		goto error;
	}

	ivpci_dev->msix_names = kmalloc(n * sizeof(*ivpci_dev->msix_names), GFP_KERNEL);
	...
	for (i = 0; i < ivpci_dev->nvectors; i++) {
		snprintf(ivpci_dev->msix_names[i], sizeof(*ivpci_dev->msix_names), "%s%d-%d", DRV_NAME, ivpci_dev->minor, i);

		ret = request_irq(ivpci_dev->msix_entries[i].vector, ivpci_interrupt, 0, ivpci_dev->msix_names[i], ivpci_dev);
		...
		dev_info(&ivpci_dev->dev->dev, PFX "irq for msix entry: %d, vector: %d\n", i, ivpci_dev->msix_entries[i].vector);
	}
...
free_names:
	...
	ivpci_dev->msix_names = NULL;

free_entries:
	kfree(ivpci_dev->msix_entries);
	ivpci_dev->msix_entries = NULL;

error:
	ivpci_dev->nvectors = 0;
	return ret;
}
...
static int ivpci_mmap(struct file *filp, struct vm_area_struct *vma)
{
	...
	if (vsize + off > len) {
		dev_err(&ivpci_dev->dev->dev, PFX "mmap overflow the end, %lu - %lu + %lu > %lu", vma->vm_end, vma->vm_start, off, len);
		ret = -EINVAL;
		goto error;
	}
...
static ssize_t ivpci_read(struct file *filp, char *buffer, size_t len, loff_t *poffset)
{
	...
	if (ivpci_dev->nvectors <= 0) {
		dev_err(&ivpci_dev->dev->dev, PFX "read: interrupts are not available\n");
		return -EOPNOTSUPP;
	}
...
static long ivpci_write(struct file *filp, const char *buffer, size_t len, loff_t *poffset)
{
	struct ivpci_private *ivpci_dev = (struct ivpci_private *)filp->private_data;
	u16 ivposition = 0;
	...
	if (ivpci_dev->nvectors <= 0) {
		dev_err(&ivpci_dev->dev->dev, PFX "write: doorbell interrupts are not available\n");
		return -EOPNOTSUPP;
	}
	...
	writel((((u32)ivposition) << 16) | (((u32)vector) & 0xFFFF), ivpci_dev->regs_addr + DOORBELL_OFF);
...
static int ivpci_probe(struct pci_dev *pdev, const struct pci_device_id *id)
{
	...
	dev_info(&pdev->dev, PFX "BAR0: 0x%0lx, %lu\n", ivpci_dev->bar0_addr, ivpci_dev->bar0_len);
	dev_info(&pdev->dev, PFX "BAR1: 0x%0lx, %lu\n", ivpci_dev->bar1_addr, ivpci_dev->bar1_len);
	dev_info(&pdev->dev, PFX "BAR2: 0x%0lx, %lu\n", ivpci_dev->bar2_addr, ivpci_dev->bar2_len);
	...
	if (device_create(g_ivpci_class, NULL, devno, NULL, DRV_FILE_FMT, ivpci_dev->minor) == NULL) {
	...
	if (ivpci_dev->revision == 1) {
		...
		if (ret != 0) {
			dev_warn(&pdev->dev,
				 PFX "MSI-X is unavailable; continuing in mmap-only mode: %d\n",
				 ret);
		}
	}
...
static int __init ivpci_init(void)
{
	...
	g_ivpci_devs = kzalloc(sizeof(struct ivpci_private) * g_max_devices, GFP_KERNEL);
