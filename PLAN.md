...
1. 与 cxlkv 使用同一套 `experiment_config.jsonc` 拓扑语义：相同
   `shared_memory.size_mb`（须为 2 的幂，PCI BAR 要求）、相同
   `shared_memory.hwcc.{offset_mb,size_mb}` /
   `shared_memory.swcc.{offset_mb,size_mb}`、相同 `shared_memory.path` /
   `device_path`、相同 VM 数/核数/NUMA 绑定、相同 worker 数、相同
   `fixed_key_size`/`fixed_value_size`、**同一份** YCSB-cpp 生成的 trace、
   相同延迟注入参数。路径取值以本仓库根目录 `experiment_config.jsonc` 为准，
   并与 cxlkv 根配置对齐，不在文档中硬编码宿主绝对路径。
...
5. 吞吐计时只覆盖 workload replay，不含初始化/checkpoint/reset/barrier；
   汇总 `ops_per_sec = ops_sum / (max(duration_us) / 1e6)`，与 cxlkv 报告
   同口径（`ops_sum` 与 `duration_sec_max` 并列输出）。
...
| 只读检查 | cxlkv 预检/环境检查语义 | `tigonkv_check_vms.sh` |

配置与产物要求：

1. 拓扑字段与 cxlkv **同名同语义**（见 §1.6）；正式对比取值**必须**等于
   §1.11 钉死数字，禁止施工时另选"差不多"的值。
2. QEMU / ivshmem / SSH / taskset 与 cxlkv **基本相同**（bash 重写；host
   tuning 默认只检查）。镜像层包装**本仓库** `emulation/image/make_vm_img.sh`
   （tigon 自带 mkosi），不调用 `../cxlkv` 的 make_img。
...
| 职责 | cxlkv 参考 | 本仓库交付 |
|------|------------|------------|
| Trace 生成器 | `thirdparty_libs/YCSB-cpp` + `scripts/generate_cxlkv_trace.sh` | 同 SHA 的 submodule（gitlink 见 §1.11）+ submodule 内同名生成脚本为唯一入口（不得依赖 `../cxlkv` 路径） |
...
| Op | 语义 |
|----|------|
| `Put(key, value)` | load：插入（重复 key hard fail）；run：upsert；**禁止 batch** |
| `Get(key)` | 读当前值；不存在则 miss；不强制校验 value 字节 |
| `Delete(key)` | 删除；之后 Get 为 miss |
| `Scan(start_key, limit)` | 从 `start_key` 起按键序最多 `limit` 条（`limit==0` 不限制）；无端键；结果为全局键序前缀（跨 partition 归并后）。若本项未通过正确性验收，则触发 §1.5.2 的 YCSB-E 例外 |
...
| `tigonkv_check_vms.sh` | 校验 cmdline / numa_maps 共享页 NUMA / taskset |
...
正式对比拓扑与引擎参数（与 cxlkv `origin/my-work` 根
`experiment_config.jsonc` 逐项相等；施工只许抄，不许挑）：
...
| 正式 YCSB | record/operation=5000000、threads_per_node=4、workloads a,b,c,d、zipfian 常数取库默认 0.99 |
| 正式延迟档 | swcc 25ns/线、hwcc 117ns/线、hwcc 原子 117ns、`per_thread_lru` 4096 线 ×8 路（同 cxlkv e2e_11 档） |
...
| 多轮吞吐主字段 | `ops_per_sec_from_avg_round_max`（p50/p90 仅附录） |
...
Scan 迁移重试 ≤8、`limit==0` 结果硬上限 1,048,576 条；GET_MISS 重转发
≤8；统计 per-thread TLS；无专职 service 线程；无 runtime rebalance；
checkpoint 不做 msync；dirty attach 一律 hard fail。
...
- **PUT**：见 4.6 分派。load 阶段 insert 语义（重复 key hard fail）；
  run 阶段 upsert。owner：latch → 若 migrated 则 shared `do_write`+
...
- **SCAN(start, limit)**（§1.5.3；抗迁移竞态）：
  ...
  3. 请求方 k 路归并取全局前 `limit`：**流式堆归并、凑满 `limit` 即早
     ...
     `limit` 条），不物化各 owner 全量结果；`limit==0` 不限制条数但硬
     上限 **1,048,576** 条结果（超出 hard fail；不新增配置键），禁止
     无限缓冲。
...
| 资产 | 处置 |
|------|------|
| `experiment_config.jsonc` + `Config::FromJsonc` | 保留；`tigon_kv` 段字段以 §1.6.1 为准（`latency_inject`、`scc_mechanism`、`when_to_move_out`、`hw_cc_budget_mb`（默认=hwcc.size_mb）、`owner_private_swcc_fraction`、`partition_count`、`transport_ring_total_mb`）；延迟字段整体替换为 cxlkv 同构 `latency_inject` 对象（见 4.9）；`tigon_kv` 内 unknown field 仍 hard fail；根级 cxlkv-only 键（`network`/`sync`/`vm.copy_root_img`/`vm.use_ivshmem_doorbell`）parse-and-ignore（§1.6.1 规则 6）；HEAD 遗留键 `hwcc_budget_mb`/`hwcc_reserved_mb`/`shared_payload_swcc_fraction` 出现即 hard fail（§1.6.1 规则 8）；`scc_mechanism` 只接受 `"WriteThrough"`、`migration_policy` 只接受 `"Clock"`、`when_to_move_out` 只接受 `"OnDemand"`，其它取值 hard fail |
...
2. `tigonkv_init_vms.sh`（对应 cxlkv `init_scripts_env_3_init_vm.fish` +
   ...
   - **每 VM QEMU 启动（照搬 cxlkv rust init_vm 的参数集，逐项保留）**：
     ...
     `EPYC,topoext`。与 cxlkv 的差异仅两处并须注释说明：(a) tap 桥接网卡
     默认省略（本项目 e2e 只用 SSH hostfwd + 共享内存 barrier；若确需 VM
     间 IP 网络，复用已存在的 tap 设备，绝不新建桥）；(b) host tuning 见上。
...
4. `tigonkv_check_vms.sh`：只读检查 QEMU cmdline、taskset、**numa_maps 中
   共享页 NUMA ∈ 配置 `shared_memory.numa_node`**、SSH、`device_path`，供
   `--skip-vm-init` 与 CI 前置。
...
- **选项集（与 cxlkv 同名同默认，差异注明）**：`--rounds`(1)、
  ...
  幂并校验空闲内存)、`--workloads`(默认 **`a,b,c,d`**；允许集合
  **`a,b,c,d,e`**)、`--no-latency`、`--cache-flush-mb`(512)、`--skip-build`、
  `--skip-vm-init`、`--skip-trace-gen`、`--skip-standalone-load`、
  `--prepare-only`。固定 4 VM，HWCC 固定 1024MB、其余给 SWCC。
...
- **步骤序列（对齐 cxlkv 指南）**：
  ...
  2. 基于 `--base-config` 生成本轮 `experiment_config_ycsb_4vm.jsonc`
     （拓扑字段与 cxlkv 正式对比取值对齐；按 `--no-latency` 开关
     `tigon_kv.latency_inject.enabled`）；
  3. 生成 trace config 与 `run_meta.json`（完整参数 + git SHA + 复现命令 +
     若适用则记录 `ycsb_e=unsupported`）；
  4. 调本仓库 YCSB-cpp 生成 load + 所选 workload（load 用 workloadc、
     zipfian；A 的 UPDATE 拆 GET+PUT；E 为 SCAN+PUT）——与 cxlkv 同一生成器
     保证 trace 逐字节可比；
  ...
  11. 调 `scripts/summarize_ycsb_experiment.py`（对齐 cxlkv 汇总：
      `ops_sum` / `duration_sec_max` / `ops_per_sec`，附
...
吞吐汇总与 cxlkv 对齐：每轮取各 node `duration_us` 的 **max**，再对轮次求
平均；应力相位不含 init/barrier/drain/EBR 排空。
...
经 `tigonkv_run_ycsb_experiment.sh` 跑 load+A/B/C/D；正式对比
**record-count=5000000** 并与 cxlkv 同一生成参数/同一份可比 trace。
...
`latency_inject.enabled=true`（基线对照行除外并显式标注）；shared∩vm
NUMA 为空；HWCC=1024MB；随 `ops_per_sec` 并列报告
...
| 拓扑/NUMA | 字段同名；正式对比 4×8、HWCC **1024MB**、`size_mb` 为 2 的幂、跨 NUMA；`check_vms` 验 shared 页 |
| 构建 | RelWithDebInfo = `-O3 -g3 -march=native -flto=full`（与 cxlkv `CxlkvBuildOptions.cmake` 一致）；**禁止**关 LTO 做正式对比 |
| worker/trace | 同 SHA YCSB-cpp；`worker{node*N+t}.txt` 命名；`fixed_key_size`/`fixed_value_size` 一致；trace 字节可互换 |
| 屏障相位 | 跨 VM 屏障 = TCP notify（cxlkv `sdl::notify` 同构）；drain/barrier 不计入应力窗口 |
| 吞吐公式 | 单轮 max-across-nodes；多轮主字段 `ops_per_sec_from_avg_round_max`（§6.3.1 口径；p50/p90 仅附录） |
| 读校验 | e2e_08 read 相位逐字节校验 value（cxlkv `VerifyValue` 同构） |
| `latency_inject` | 字段全集同构；仅 RelWithDebInfo+verbose=false+extra_check=false 可 enable；脚本接受顶层或 `tigon_kv.` 路径 |
| Scan 合同 | `(start, limit)`，`limit==0` 不限制；trace 无端键（cxlkv API 的 `end_exclusive` 在 trace 中恒空，兼容）；E 规则见 §1.5.2 |
...
13. §七 文档与死代码清理完成；checkpoint 不把 msync 等同 SCC；§1.10 有里程碑
    核对记录。
...
| M10 收尾 | 性能自查（分配器/转发/延迟 wrapper 热点 perf 采样，必要微调）、§7.1 文档更新、§7.2 死代码清理、验收清单逐条核对 | §6.0 + §6.4 全满足 |
